...
function getBalanceIndex(arr) {
  ...
  for (let i = 1; i < arrLen - 1; i += 1) {
    let sumLeft = 0;
    let sumRigth = 0;
    for (let y = 0; y < i; y += 1) {
      sumLeft += arr[y];
    }
    for (let z = i + 1; z < arrLen; z += 1) {
      sumRigth += arr[z];
    }
...
function getSpiralMatrix(size) {
  ...
  let right = size - 1;
  let mElement = 1;
  while (mElement <= size * size) {
    for (let i = left; i <= right; i += 1) {
      matrix[top][i] = mElement;
      mElement += 1;
    }
    top += 1;
    for (let i = top; i <= bottom; i += 1) {
      matrix[i][right] = mElement;
      mElement += 1;
    }
    right -= 1;
    for (let i = right; i >= left; i -= 1) {
      matrix[bottom][i] = mElement;
      mElement += 1;
    }
    bottom -= 1;
    for (let i = bottom; i >= top; i -= 1) {
      matrix[i][left] = mElement;
      mElement += 1;
    }
    left += 1;
  }
...
function rotateMatrix(matrix) {
  ...
  const lenX = cloneMatrix[0].length;
  const lenY = cloneMatrix.length;
  for (let i = 0; i < lenY; i += 1) {
    for (let j = 0; j < lenX; j += 1) {
      const newI = lenY - i - 1;
      newMatrix[j][i] = cloneMatrix[newI][j];
    }
  }
  return newMatrix;
}
...
function sortByAsc(arr) {
  ...
  for (let i = 1; i < arrLen; i += 1) {
    const curElement = sortedArr[i];
    let j = i - 1;
    while (j >= 0 && sortedArr[j] > curElement) {
      sortedArr[j + 1] = sortedArr[j];
      j -= 1;
    }
    sortedArr[j + 1] = curElement;
  }
  return sortedArr;
}
...
function shuffleChar(str, iterations) {
  ...
  for (let i = 1; i <= iterations; i += 1) {
    ...
    for (let j = 1; j < strLen; j += 1) {
      if (j % 2 === 0) {
        leftStr += shuffleStr[j];
      } else {
        rightStr += shuffleStr[j];
      }
    }
